...
local function main()
	...
	function RemoteSpyFunctionality()
		...
		local function EventMain(Event)
			Logger[Event]=0
			connections[Event] = connections[Event] or {}
			local conn = Event.OnClientEvent:Connect(function(...)
				if Logger[Event]>Limits[Event.ClassName] then return end
				Logger[Event]=Logger[Event]+1
				local StrArgs = tableloop({...})
				local FullData = string.format('--[[ Script generated by RemoteSpy %s]]\n-- Remote Type: %s \n\nlocal args = %s\n\n%s:FireClient(game.Players.%s, unpack(args));', vers, Event.ClassName, StrArgs, GetFullName(Event), game.Players.LocalPlayer.Name)
				Output(FullData)
				task.delay(1,function() Logger[Event]=Logger[Event]-1 end)
			end)
			table.insert(connections[Event], conn)
		end

		local function BEventMain(Event)
			Logger[Event]=0
			connections[Event] = connections[Event] or {}
			local conn = Event.Event:Connect(function(...)
				...
				Logger[Event]=Logger[Event]+1
				local StrArgs = tableloop({...})
				local FullData = string.format('--[[ Script generated by RemoteSpy %s]]\n-- Remote Type: %s \n\nlocal args = %s\n\n%s:Fire(unpack(args));', vers, Event.ClassName, StrArgs, GetFullName(Event))
				Output(FullData)
				task.delay(1,function() Logger[Event]=Logger[Event]-1 end)
			end)
			table.insert(connections[Event], conn)
		...
		local function FunctionMain(Func)
			Logger[Func] = 0
			connections[Func] = connections[Func] or {}
			local oldInvoke = Func.OnClientInvoke
			Func.OnClientInvoke = function(...)
				if Logger[Func] > Limits[Func.ClassName] then return '1' end
				Logger[Func] = Logger[Func] + 1
				local StrArgs = tableloop({...})
				local FullData = string.format('--[[ Script generated by RemoteSpy %s]]\n-- Remote Type: %s\n\nlocal args = %s\n\n%s:InvokeClient(game.Players.%s, unpack(args));',
					vers, Func.ClassName, StrArgs, GetFullName(Func), game.Players.LocalPlayer.Name)
				Output(FullData)
				task.delay(1, function() Logger[Func] = Logger[Func] - 1 end)
				return '1'
			end
			table.insert(connections[Func], {
				revert = function()
					Func.OnClientInvoke = oldInvoke
				end
			})
		end

		local function BFunctionMain(Func)
			Logger[Func] = 0
			connections[Func] = connections[Func] or {}
