...
local Explorer, Properties, ScriptViewer, Script, ModelViewer, Console, RemoteSpy, Notebook -- Major Apps
...
local function main()
	...
	Lib.Window = (function()
		...
		local function renderSide(side, noTween) -- change old to new ( i hope it work )
		    ...
		    for _, conn in pairs(side.ResizeCons) do
		        conn:Disconnect()
		    end
		    side.ResizeCons = {}
		
		    local existingResizers = {}
		    for _, child in pairs(side.Frame:GetChildren()) do
		        if child.Name == "WindowResizer" then
		            table.insert(existingResizers, child)
		        end
		    end
		
		    local neededResizers = math.max(0, #side.Windows - 1)
		
		    if neededResizers < #existingResizers then
		        for i = neededResizers + 1, #existingResizers do
		            if existingResizers[i] then
		                existingResizers[i]:Destroy()
		                existingResizers[i] = nil
		            end
		        end
		    end
		
		    for i = #existingResizers + 1, neededResizers do
		        local newResizer = baseTemplate:Clone()
		        newResizer.Name = "WindowResizer"
		        newResizer.Parent = side.Frame
		        existingResizers[i] = newResizer
		    end
		    ...
		    for i, v in pairs(side.Windows) do
		        ...
		        if not isEnd then
		            local resizer = existingResizers[i]
		            if resizer then
		                resizer.Position = UDim2.new(1, -side.Width, 0, currentPos - 4)
		                side.ResizeCons[#side.ResizeCons + 1] = v.Gui.Main:GetPropertyChangedSignal("Size"):Connect(function()
		                    resizer.Position = UDim2.new(1, -side.Width, 0, v.GuiElems.Main.Position.Y.Offset + v.GuiElems.Main.Size.Y.Offset)
		                end)
		                side.ResizeCons[#side.ResizeCons + 1] = v.Gui.Main:GetPropertyChangedSignal("Position"):Connect(function()
		                    resizer.Position = UDim2.new(1, -side.Width, 0, v.GuiElems.Main.Position.Y.Offset + v.GuiElems.Main.Size.Y.Offset)
		                end)
		
		                sideResizerHook(resizer, "V", side, i)
		            end
		        end
		    end
...
-- Main vars
local Main, Explorer, Properties, ScriptViewer, Script, ModelViewer, Console, RemoteSpy, DefaultSettings, Notebook, Serializer, Lib
local API, RM
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer","Script","ModelViewer","Console","RemoteSpy"}
	Main.Elevated = false
	Main.MissingEnv = {}
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			...
			rawXML = rawXML or game:HttpGet("https://raw.githubusercontent.com/BOXLEGENDARY/ReflectionMetadata/refs/heads/main/ReflectionMetadata.xml")
		else
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{2,"Frame",{Active=true,BackgroundColor3=Color3.new(0.05,0.05,0.05),BorderSizePixel=0,Name="Main",Parent={1},Position=UDim2.new(0.5,-175,0.5,-100),Size=UDim2.new(0,350,0,200),}},
			{3,"Frame",{BackgroundColor3=Color3.new(0.15,0.15,0.15),BorderSizePixel=0,ClipsDescendants=true,Name="Holder",Parent={2},Size=UDim2.new(1,0,1,0),}},
			{4,"UIGradient",{Parent={3},Rotation=30,Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.1),NumberSequenceKeypoint.new(1,0.1)}),}},
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0.5,0,0,15),AnchorPoint=Vector2.new(0.5,0),Size=UDim2.new(0,100,0,50),Text="ZDex",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=0,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0.5,0,0,70),AnchorPoint=Vector2.new(0.5,0),Size=UDim2.new(0,180,0,25),Text="Debugging Suite",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=0,}},
			{7,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="StatusText",Parent={3},Position=UDim2.new(0.5,0,0,115),AnchorPoint=Vector2.new(0.5,0),Size=UDim2.new(0,180,0,25),Text="Fetching API",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0,}},
			{8,"Frame",{BackgroundColor3=Color3.new(0.25,0.25,0.25),BorderSizePixel=0,Name="ProgressBar",Parent={3},Position=UDim2.new(0.5,-120,0,150),Size=UDim2.new(0,240,0,4),AnchorPoint=Vector2.new(0,0),}},
			{9,"Frame",{BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Name="Bar",Parent={8},Size=UDim2.new(0,0,1,0),}},
			{10,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://2764171053",ImageColor3=Color3.new(0.15,0.15,0.15),Parent={8},ScaleType=1,Size=UDim2.new(1,0,1,0),SliceCenter=Rect.new(2,2,254,254),}},
			...
			{12,"UIGradient",{Parent={11},Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.1),NumberSequenceKeypoint.new(1,0.1)}),}},
			...
			{14,"UIGradient",{Parent={13},Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.1),NumberSequenceKeypoint.new(1,0.1)}),}},
			...
			{16,"UIGradient",{Parent={15},Rotation=-30,Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.1),NumberSequenceKeypoint.new(1,0.1)}),}},
			{17,"UIGradient",{Parent={2},Rotation=-30,Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.1),NumberSequenceKeypoint.new(1,0.1)}),}},
		})
		Main.ShowGui(gui)
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{6,"Frame",{BackgroundColor3=Color3.fromRGB(20,20,20),Name="BottomFrame",Parent={4},Position=UDim2.new(0,0,1,-28),Size=UDim2.new(1,0,0,28)}},
			{7,"UICorner",{CornerRadius=UDim.new(0,4),Parent={6}}},
			{8,"Frame",{BackgroundColor3=Color3.fromRGB(40,40,40),BorderSizePixel=0,Name="CoverFrame",Parent={6},Size=UDim2.new(1,0,0,4)}},
			{9,"Frame",{BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0,Name="Line",Parent={8},Position=UDim2.new(0,0,0,-1),Size=UDim2.new(1,0,0,1)}},
			...
			{11,"ImageLabel",{BackgroundTransparency=1,Image="rbxassetid://6578871732",ImageTransparency=0.2,Name="Icon",Parent={10},Position=UDim2.new(0,4,0,4),Size=UDim2.new(0,16,0,16)}},
			...
			{13,"ImageLabel",{BackgroundTransparency=1,Image="rbxassetid://6578933307",ImageTransparency=0.2,Name="Icon",Parent={12},Position=UDim2.new(0,4,0,4),Size=UDim2.new(0,16,0,16)}},
			{14,"ScrollingFrame",{Active=true,AnchorPoint=Vector2.new(0.5,0),BackgroundTransparency=1,BorderSizePixel=0,Name="AppsFrame",Parent={4},Position=UDim2.new(0.5,0,0,0),ScrollBarImageColor3=Color3.fromRGB(255,255,255),ScrollBarThickness=4,Size=UDim2.new(0,236,1,-30)}},
			...
			{18,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(40,40,40),BorderSizePixel=0,Font=Enum.Font.GothamBold,Name="Main",Parent={17},Size=UDim2.new(1,0,0,60),Text="",TextColor3=Color3.fromRGB(255,255,255),TextSize=14}},
			...
			{20,"TextLabel",{BackgroundTransparency=1,Font=Enum.Font.GothamSemibold,Name="AppName",Parent={18},Position=UDim2.new(0,2,0,38),Size=UDim2.new(1,-4,1,-40),Text="Explorer",TextColor3=Color3.fromRGB(255,255,255),TextSize=14,TextTransparency=0,TextWrapped=true}},
			{21,"Frame",{BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0,Name="Highlight",Parent={18},Position=UDim2.new(0,0,1,-2),Size=UDim2.new(1,0,0,2)}}
		})
		Main.MainGui = gui
