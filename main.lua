...
Main = (function()
	...
	local USE_IN_STUDIO = false -- Toggles Roblox Studio mode, which allows for this to be used in]]
	        ...
	        local code = Decompile_Source:gsub(";;CONSTANTS HERE;;", CONSTANTS)
	        code = code:gsub("return %%(x %%%% 2%^32%%) // %%(2%^disp%%)", "return math.floor((x %% 2^32) / (2^disp))")
	        
	        local f, err = loadstring(code, "Advanced-Luau-Decompiler")
	        if f then
	            setfenv(f, getgenv())
	            pcall(f)
	        end
